...
serve(async (req) => {
  ...
  try {
    ...
    // Input validation - limit theme length
    const sanitizedTheme = theme ? theme.trim().slice(0, 200) : '';
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Tema: ${sanitizedTheme || 'Crie posts sobre tendências atuais de marketing digital'}` }
        ],
        temperature: 0.8,
        max_tokens: 1000
      })
